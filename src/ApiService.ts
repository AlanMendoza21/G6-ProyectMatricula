...
import {
  ListRep,
  Matricula,
  RespuestaCursoDisponibleEstudiante,
  RespuestaSeccion,
  RespuestaUsuario,
  Turno,
  Turnos,
...
export class ApiService {
  ...
  reporteMatricula(data:string): Observable<ListRep> {
    return this.http.post<ListRep>('http://localhost:8080/obtenerReporteMatricula', data,
        this.httpOptions)
        .pipe(
            retry(1),
            catchError(this.errorHandl)
